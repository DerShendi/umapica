...
public class DebugScreenEntries {
    ...
    private static Identifier register(String name, DebugScreenEntry entry) {
        return register(Identifier.withDefaultNamespace(name), entry);
    }

    private static Identifier register(Identifier name, DebugScreenEntry entry) {
        ENTRIES_BY_ID.put(name, entry);
        return name;
    }
    ...
    public static @Nullable DebugScreenEntry getEntry(Identifier name) {
        return ENTRIES_BY_ID.get(name);
    }
    ...
    static {
        ...
        // Neo: Allow registering custom entries
        PROFILES = net.neoforged.fml.ModLoader.postEventWithReturn(new net.neoforged.neoforge.client.event.RegisterDebugEntriesEvent(ENTRIES_BY_ID, map, map1)).validateProfiles();
    }
}
