...
public abstract class Screen extends AbstractContainerEventHandler implements Renderable {
    ...
    protected Screen(Minecraft minecraft, Font font, Component title) {
        ...
        this.font = font;
        this.title = title;
        this.screenExecutor = p_454168_ -> minecraft.execute(() -> {
    ...
    protected static boolean clickUrlAction(Minecraft minecraft, @Nullable Screen screen, URI url) {
        if (!minecraft.options.chatLinks().get()) {
            ...
            if (minecraft.options.chatLinksPrompt().get()) {
                minecraft.setScreen(new ConfirmLinkScreen(p_465510_ -> {
                    if (p_465510_) {
    ...
    public static void renderMenuBackgroundTexture(
        GuiGraphics guiGraphics, Identifier texture, int x, int y, float uOffset, float vOffset, int width, int height
    ) {
        int i = 32;
    ...
    public static Screen.@Nullable NarratableSearchResult findNarratableWidget(List<? extends NarratableEntry> entries, @Nullable NarratableEntry target) {
